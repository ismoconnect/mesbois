  ...
  return new Promise((resolve) => {
    try {
      ...
      const req = lib.request(parsed, options, (res) => {
        ...
        // destroy to not download full body
        req.destroy();
...
async function run() {
  console.log('🔎 Vérification GET des URLs d\'images (settings/productImages)');
  try {
    ...
    console.log(`🔢 Vérification GET d'un échantillon de ${sample.length} URLs (premiers éléments)`);
